...
func GetDailyStatements(c *fiber.Ctx, col *mongo.Collection) error {
    ...
    data, err := col.Aggregate(ctx, pipeline)

    if err != nil {
        log.Fatal(err)
    }

    log.Println("Aggregation result count:", data.RemainingBatchLength())
    ...
    log.Println("Response count:", len(response))
